/* ============================================================
   SONRISA DENTAL — Citas en línea (calendario demo)
   ============================================================ */

(function () {
  "use strict";

  var WHATSAPP_NUMBER = "529601427950"; // NÚMERO A REEMPLAZAR por el del consultorio
  ...
  var view = new Date();        // mes en pantalla
  view.setDate(1);
  ...
  function renderCalendar() {
    ...
    var offset = (first.getDay() + 6) % 7;                    // semana inicia en lunes
  ...
  calGrid.addEventListener("click", function (ev) {
    ...
    selection.slot = null;          // cambió el día, se limpia la hora
    renderCalendar();
    renderSlots();
    updateSummary();
  });
  ...
  function updateSummary() {
    ...
    var priceFmt = pr === "Desde $4,500" || pr === "Desde $3,000"
      ? pr
      : "$" + Number(pr).toLocaleString("es-MX");
    summary.innerHTML =
      "<strong>" + svc + "</strong> · " + priceFmt +
  ...
  confirmBtn.addEventListener("click", function () {
    ...
    var priceFmt = selection.price === "Desde $4,500" || selection.price === "Desde $3,000"
      ? selection.price
      : "$" + Number(selection.price).toLocaleString("es-MX");

    var texto =
      "Hola, quiero agendar una cita dental en Sonrisa Dental." +
      "\n\n• Servicio: " + selection.service + " (" + priceFmt + ")" +
      "\n• Fecha: " + fmtDate(selection.date) +
      "\n• Hora: " + selection.slot +
      "\n\nMi nombre es: " + "___" +
      "\n\n¿Me confirman la cita?";
